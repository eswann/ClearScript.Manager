...
namespace ClearScript.Manager.Caching
{
    ...
    public class LruCache<TKey, TValue> : IConcurrentDictionary<TKey, TValue>
    {
        ...
        /// <summary>Creates a LurchTable that can store up to (capacity) items efficiently.</summary>
        public LruCache(int capacity, LurchTableOrder ordering)
            : this(ordering, int.MaxValue, capacity >> 1, capacity >> 4, capacity >> 8, EqualityComparer<TKey>.Default) { }

        /// <summary>Creates a LurchTable that can store up to (capacity) items efficiently.</summary>
        public LruCache(int capacity, LurchTableOrder ordering, IEqualityComparer<TKey> comparer)
        ...
        public LruCache(LurchTableOrder ordering, int limit)
            : this(ordering, limit, limit >> 1, limit >> 4, limit >> 8, EqualityComparer<TKey>.Default) { }

        /// <summary>Creates a LurchTable that orders items by (ordering) and removes items once the specified (limit) is reached.</summary>
        public LruCache(LurchTableOrder ordering, int limit, IEqualityComparer<TKey> comparer)
        ...
        {
            private readonly LruCache<TKey, TValue> _owner;
            private EnumState _state;

            internal Enumerator(LruCache<TKey, TValue> owner)
            {
                _owner = owner;
        ...
        public class KeyCollection : ICollection<TKey>
        {
            ...
            internal KeyCollection(LruCache<TKey, TValue> owner)
            {
                _owner = owner;
            }
        ...
        public class ValueCollection : ICollection<TValue>
        {
            ...
            internal ValueCollection(LruCache<TKey, TValue> owner)
            {
                _owner = owner;
            }
